...
watch(gastos, () => {
  const totalGastado = gastos.value.reduce((acumulador, item) => { return item.cantidad + acumulador }, 0);
...
const guardarGasto = () => {
  if (gasto.id) {
    const { id } = gasto;
    const index = gastos.value.findIndex(item => item.id === id);
...
const eliminarGasto = () => {
  if (confirm('Eliminar?')) {
    gastos.value = gastos.value.filter(item => item.id !== gasto.id);
    ocultarModal();
  }
};
  ...
  <div :class="{
    ...
    <header>
      ...
      <div class="contenedor-header contenedor sombra">
        ...
        <ControlPresupuesto :presupuesto="presupuesto" :disponible="disponible" :gastado="gastado" @reset-app="resetApp"
          v-else />
      </div>
    </header>
    ...
    <main v-if="presupuesto > 0">
      ...
      <Modal v-if="modal.mostrar" :modal="modal" :disponible="disponible" :id="gasto.id" @ocultar-modal="ocultarModal"
        @guardar-gasto="guardarGasto" @eliminar-gasto="eliminarGasto" v-model:nombre="gasto.nombre"
